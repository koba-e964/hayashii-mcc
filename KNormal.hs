...
type KNormal = Typed KNormalExpr
data KNormalExpr
  = KUnit
  | KInt !Int
  | KFloat !Float
...
freeVars (KIf _ x y (e1 :-: _) (e2 :-: _)) = Set.fromList [x,y] `union` freeVars e1 `union` freeVars e2
freeVars (KLet name_ _ (e1 :-: _) (e2 :-: _)) = Set.delete name_ (freeVars e2) `union` freeVars e1
freeVars (KVar x) = Set.singleton x
freeVars (KLetRec (KFundef {name = (x,_), args = a, body = b :-: _}) (expr :-: _)) =
...
freeVars (KLetTuple decs i (e :-: _)) = Set.singleton i `union` (freeVars e `difference` Set.fromList (map fst decs))
...
-- inserts let
insertLet :: Monad m => KNormal -> (Id -> CounterT m KNormal) -> CounterT m KNormal
insertLet te@(e :-: t) k =
  case e of
    KVar x -> k x
    _ -> do
      x <- Id.genTmp t
      e'@(_ :-: t') <- k x
      return (KLet x t te e' :-: t')


-- knormal routine. This routine normalizes expressions and adds types to them.
kNormalSub :: (Monad m, MonadReader TypeEnv m) => Map Id Type -> Syntax -> CounterT m KNormal
kNormalSub env expr = case expr of
  ...
  If (Cmp cmp e1 e2) etr efl -> do -- condition with comparison
      ...
      insertLet k1
        (\x -> insertLet k2
            (\y -> do
               etr'@(_ :-: ttr) <- kNormalSub env etr
               efl' <- kNormalSub env efl
               return (KIf cmp x y etr' efl' :-: ttr)
            ))
  If e1 etr efl -> kNormalSub env (If (Cmp Eq e1 (Bool False)) etr efl) -- others
  Let x t e1 e2 -> do
      e1' <- kNormalSub env e1
      e2'@(_ :-: t2) <- kNormalSub (Map.insert x t env) e2
  ...
  LetTuple xts e1 e2 -> do
      ...
      insertLet k1
        (\ y -> do
          e2'@(_ :-: t2) <- kNormalSub (Map.fromList xts `Map.union` env) e2
